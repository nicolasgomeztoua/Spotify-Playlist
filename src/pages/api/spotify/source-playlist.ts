...
import { SpotifyApi, type AccessToken, type Track, type PlaylistedTrack, type AudioFeatures } from "@spotify/web-api-ts-sdk";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const initialAccessToken = {
      ...
      token_type: "Bearer",
      expires_in: token.expiresAt ? Math.max(0, Math.floor(token.expiresAt - Date.now() / 1000)) : 3600,
      refresh_token: token.refreshToken ?? "",
      expires_at: token.expiresAt ?? 0,
    };
    const initialSpotify = SpotifyApi.withAccessToken(clientId, initialAccessToken);
    ...
    while (hasMoreTracks) {
      try {
        const playlistTracks = await initialSpotify.playlists.getPlaylistItems(playlistId, undefined, undefined, limit, offset); // Use initialSpotify here
        ...
        if (!playlistTracks.next) { // Check if there's a next page URL
          hasMoreTracks = false;
        }
      } catch (error) {
        console.error("Error fetching playlist tracks page:", error);
        // Stop pagination on error and potentially inform the user
        return res.status(500).json({ error: "Failed to fetch all playlist tracks." });
      }
    }
    ...
    for (let i = 0; i < trackIds.length; i += 100) {
      ...
      try {
        ...
        const validFeatures = featuresResult.filter((f): f is AudioFeatures => f !== null);
        allAudioFeatures.push(...validFeatures);
      } catch (error) {
        console.error(`Error fetching audio features for batch ${Math.floor(i / 100) + 1}:`, error); // Keep error log
        const message = error instanceof Error ? error.message : "Failed to fetch audio features for a batch";
        const statusCode = message.includes("403") || message.includes("Bad OAuth") ? 403 : 500;
        // Stop processing if any batch fails
        return res.status(statusCode).json({ error: `Analysis failed during audio feature fetch: ${message}` }); 
      }
    }
    ...
    const message = error instanceof Error ? error.message : "Failed to analyze playlist";
